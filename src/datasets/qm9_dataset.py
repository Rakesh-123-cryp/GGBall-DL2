...
class QM9DataModule(MolecularDataModule):
    def __init__(self, cfg):
        ...
        self.batch_size = getattr(cfg.train, 'batch_size')
        self.debug = cfg.dataset.debug
        if self.debug: 
            self.drop_last = getattr(cfg.dataset, 'drop_last', True) 
            self.shuffle = getattr(cfg.dataset, 'shuffle', True)
        else:
        ...
        # train_QM9 = QM9Dataset(stage='train', root=root_path, remove_h=cfg.dataset.remove_h,
        #                                 target_prop=target, transform=RemoveYTransform())
        # subset_indices = list(range(4))
        # from torch.utils.data import Subset
        # QM9_small = Subset(train_QM9, subset_indices)
        # datasets = {'train': QM9_small,
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            shuffle=self.shuffle,  # 使用self.shuffle参数
...
def get_test_smiles(cfg, test_dataloader, dataset_infos, evaluate_dataset=False):
    ...
    if os.path.exists(smiles_path):
        ...
        print("Computing dataset smiles...")
        train_smiles = compute_qm9_smiles(atom_decoder, test_dataloader, remove_h)
...
def compute_qm9_smiles(atom_decoder, train_dataloader, remove_h):
    ...
    for i, data in enumerate(train_dataloader):
        dense_data, node_mask = utils.to_dense(data.x, data.edge_index, data.edge_attr, data.batch)
        dense_data = dense_data.mask_argmax(node_mask, collapse=True)
        X, E = dense_data.X, dense_data.E
